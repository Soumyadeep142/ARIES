...
for no in range(2,8):
	...
	for Target in Tar_idn:
		...
		if SD_Tar>=3*SD_com:
			...
			Time_new_list=list(Time_new_list)
			Mag_new_list=list(Mag_new_list)
			Err_new_list=list(Err_new_list)
			for j in range(len(Time_new_list)):
				Time_new_list[j]=Time_new_list[j]-2450000
			Time_new_list.insert(0, 'JD(2450000+)')
			Mag_new_list.insert(0, 'Magnitude')
			Err_new_list.insert(0, 'Error')
			table=[[Time_new_list[k], Mag_new_list[k],Err_new_list[k]] for k in range(len(Time_new_list))]
			with open('{0}.txt'.format(int(Target)), 'w') as f:
